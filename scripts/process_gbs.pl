...
sub picard_duplicate_marking
{
	...
	map{
		...
		push @rmdup_bams, $out;
		my $cmd = "java -Xmx15G -Djava.io.tmpdir=./  -jar $picard_dir/MarkDuplicates.jar I=$bam O=$out M=$merics_file REMOVE_DUPLICATES=false AS=true";
		print "$cmd failed \n" if system($cmd);
	}@bams;
}
...
sub GATK_realignment
{
	...
	map {
		...
		my $cmd = "java -Xmx15G -jar $GATK_jar -I $_ -R $ref_fasta -T RealignerTargetCreator -o $interval";
		print  "$cmd failed\n" if system($cmd);
	...
	map{
		...
		my $cmd = "java -Xmx15G -jar $GATK_jar -I $_ -R $ref_fasta -T IndelRealigner -targetIntervals $interval -o $realn_bam";
		print  "$cmd failed\n" if system($cmd);
